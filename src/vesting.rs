...
pub enum VestingCurve {
    Linear,
    Cliff,
    /// Graded vesting: step_secs is the duration of each step.
    /// Stored as tuple variant (step_secs: u64).
    Graded(u64),
    /// Step vesting: steps is the number of equal steps.
    /// Stored as tuple variant (steps: u32).

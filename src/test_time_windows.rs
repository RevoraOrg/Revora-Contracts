...
fn report_window_unset_always_open() {
    ...
    RevoraRevenueShareClient::new(&env, &cid).register_offering(&issuer, &Vec::new(&env), &1u32, &symbol_short!("ns"), &token, &1_000, &token, &0);
...
fn report_window_is_scoped_per_offering() {
    ...
    RevoraRevenueShareClient::new(&env, &cid).register_offering(&issuer, &Vec::new(&env), &1u32, &symbol_short!("ns"), &token_a, &1_000, &token_a, &0);
    RevoraRevenueShareClient::new(&env, &cid).register_offering(&issuer, &Vec::new(&env), &1u32, &symbol_short!("ns"), &token_b, &1_000, &token_b, &0);
...
fn claim_window_is_scoped_per_offering() {
    ...
    RevoraRevenueShareClient::new(&env, &cid).register_offering(
        &issuer,
    &Vec::new(&env),
    &1u32, &symbol_short!("ns"), &token_a, &10_000, &payment_token, &0,
    );
    RevoraRevenueShareClient::new(&env, &cid).register_offering(
        &issuer,
    &Vec::new(&env),
    &1u32, &symbol_short!("ns"), &token_b, &10_000, &payment_token, &0,
    );
    mint(&env, &payment_token, &issuer, 10_000_000);

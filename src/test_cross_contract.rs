...
fn cross_contract_registry_check_before_offering() {
    ...
    assert!(registry.is_approved(&token));
    revora.register_offering(&issuer, &Vec::new(&env), &1u32, &symbol_short!("def"), &token, &5_000, &payout_asset, &0);
...
fn cross_contract_full_workflow_registry_then_offering() {
    ...
    revora.register_offering(&issuer, &Vec::new(&env), &1u32, &symbol_short!("def"), &token, &3_000, &payout_asset, &0);
...
fn cross_contract_concurrent_state_changes() {
    ...
    revora.register_offering(&issuer, &Vec::new(&env), &1u32, &symbol_short!("def"), &token_1, &1_000, &payout_1, &0);

    registry.register_token(&admin, &token_2);
    revora.register_offering(&issuer, &Vec::new(&env), &1u32, &symbol_short!("def"), &token_2, &2_000, &payout_2, &0);

    registry.register_token(&admin, &token_3);
    revora.register_offering(&issuer, &Vec::new(&env), &1u32, &symbol_short!("def"), &token_3, &3_000, &payout_3, &0);

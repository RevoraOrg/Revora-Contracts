...
fn test_namespace_isolation() {
    ...
    client.register_offering(&issuer_a, &Vec::new(&env), &1u32, &ns_1, &token, &1000, &token, &0);
    // Issuer B registers in ns2 with SAME token
    client.register_offering(&issuer_b, &Vec::new(&env), &1u32, &ns_2, &token, &2000, &token, &0);
...
fn test_same_issuer_different_namespaces() {
    ...
    client.register_offering(&issuer, &Vec::new(&env), &1u32, &ns_1, &token, &1000, &token, &0);
    client.register_offering(&issuer, &Vec::new(&env), &1u32, &ns_2, &token, &2000, &token, &0);
...
fn test_cross_namespace_blacklist_isolation() {
    ...
    client.register_offering(&issuer, &Vec::new(&env), &1u32, &ns_1, &token, &1000, &token, &0);
    client.register_offering(&issuer, &Vec::new(&env), &1u32, &ns_2, &token, &1000, &token, &0);
...
fn test_unauthorized_issuer_access_fails() {
    ...
    client.register_offering(&issuer_real, &Vec::new(&env), &1u32, &ns_1, &token, &1000, &token, &0);
...
fn test_transfer_maintains_namespace_isolation() {
    ...
    client.register_offering(&issuer_a, &Vec::new(&env), &1u32, &ns_1, &token_1, &1000, &token_1, &0);
...
fn test_duplicate_registration_fails() {
    ...
    let res = client.try_register_offering(&issuer, &Vec::new(&env), &1u32, &ns, &token, &1000, &token, &0);
    assert!(res.is_err());
}
...
fn test_aggregation_across_namespaces() {
    ...
    client.initialize(&issuer, &None::<Address>, &None::<bool>);
    client.register_offering(&issuer, &Vec::new(&env), &1u32, &ns_1, &token1, &1000, &token1, &0);
    client.register_offering(&issuer, &Vec::new(&env), &1u32, &ns_2, &token2, &1000, &token2, &0);
...
fn test_blacklist_precedence_over_whitelist() {
    ...
    client.initialize(&issuer, &None::<Address>, &None::<bool>);
    client.register_offering(&issuer, &Vec::new(&env), &1u32, &ns, &token, &1_000, &payout_asset, &0);

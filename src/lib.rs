...
impl RevoraRevenueShare {
    ...
    /// Only the platform admin may call this. Defaults to 0 if not set.
    /// The platform fee is taken from reported revenue for platform-level revenue sharing.
    pub fn set_platform_fee(env: Env, fee_bps: u32) -> Result<(), RevoraError> {
        ...
        let fee_key = DataKey::PlatformFeeBps;
        env.storage().persistent().set(&fee_key, &fee_bps);
        Ok(())
    }

    /// Return the current platform fee in basis points.
    /// Returns 0 if not set (default).
    ...
    /// Uses the currently configured platform fee in basis points.
    /// Returns 0 if platform fee is not set or is 0.

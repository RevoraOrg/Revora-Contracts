...
pub enum RevoraError {
    ...
    DisputeZeroShare = 61,
    /// NAV per share must be strictly positive for auto-reinvestment.
    AutoReinvestInvalidNav = 62,
    /// Auto-reinvestment is not enabled for this holder.
    AutoReinvestNotEnabled = 63,
}
...
/// Emitted when a holder's dividend is auto-reinvested as additional shares.
const EVENT_DIVIDEND_REINVEST: Symbol = symbol_short!("div_rein");
...
/// Per-holder auto-reinvestment configuration for an offering.
///
/// When `enabled` is `true`, the `claim` function converts the owed dividend
/// into additional basis-point shares instead of transferring tokens.
///
/// ### NAV precision
/// `nav_per_share_e7` is the net-asset-value per one basis-point share expressed
/// in the offering's payment-token units at 7-decimal (canonical Stellar) precision.
/// For example, if 1 bps-share is worth 0.05 USDC (5_000_000 in 7-decimal units),
/// set `nav_per_share_e7 = 5_000_000`.
///
/// ### Share-delta formula
/// ```
/// share_delta_bps = total_payout_e7 / nav_per_share_e7
/// ```
/// Fractional bps are truncated (floor division) to remain conservative.
/// The resulting `share_delta_bps` must be ≥ 1 for the reinvestment to be
/// applied; otherwise the call succeeds but acts as a normal zero-transfer claim
/// (the tiny fractional amount is left unclaimed and the period index still
/// advances).
#[contracttype]
#[derive(Clone, Debug, PartialEq)]
pub struct AutoReinvestConfig {
    /// Whether auto-reinvestment is active for this holder.
    pub enabled: bool,
    /// NAV per basis-point share in 7-decimal payment-token units (must be > 0).
    pub nav_per_share_e7: i128,
}
...
pub enum DataKey2 {
    ...
    // ── Dividend auto-reinvestment (feat/dividend-reinvest) ──
    /// Per-holder auto-reinvestment configuration: enabled flag + NAV per share.
    AutoReinvest(OfferingId, Address),
}
...
impl RevoraRevenueShare {
    ...
    pub fn claim(
        ...
        // ── Auto-reinvestment fork ─────────────────────────────────────────────
        // When the holder has opted in, convert the owed dividend into additional
        // basis-point shares at the configured NAV instead of transferring tokens.
        let reinvest_cfg: Option<AutoReinvestConfig> = env
            .storage()
            .persistent()
            .get(&DataKey2::AutoReinvest(offering_id.clone(), holder.clone()));

        if let Some(cfg) = reinvest_cfg {
            if cfg.enabled && total_payout > 0 && cfg.nav_per_share_e7 > 0 {
                // share_delta_bps = floor(total_payout / nav_per_share_e7)
                let share_delta = total_payout.checked_div(cfg.nav_per_share_e7).unwrap_or(0);

                if share_delta > 0 && share_delta <= u32::MAX as i128 {
                    let share_delta_bps = share_delta as u32;
                    let current_share_bps: u32 = env
                        .storage()
                        .persistent()
                        .get(&DataKey::HolderShare(offering_id.clone(), holder.clone()))
                        .unwrap_or(0);
                    // Cap new share at 100 % (10 000 bps).
                    let new_share_bps =
                        current_share_bps.saturating_add(share_delta_bps).min(10_000);
                    let effective_delta = new_share_bps.saturating_sub(current_share_bps);

                    if effective_delta > 0 {
                        // set_holder_share_internal enforces supply-cap and vesting rules.
                        // On failure (cap exhausted / vesting blocked) we fall through to
                        // the normal token transfer so the holder still receives cash.
                        if Self::set_holder_share_internal(
                            &env,
                            offering_id.issuer.clone(),
                            offering_id.namespace.clone(),
                            offering_id.token.clone(),
                        ...
                        .is_ok()
                        {
                            env.events().publish(
                                ...
                                (
                                    ...
                                    total_payout,
                                    effective_delta,
                                    cfg.nav_per_share_e7,
                                ),
                            );
                            // Suppress token transfer — dividend reinvested as shares.
                            total_payout = 0;
                        }
                    }
                }
        ...
        // Transfer only if there is a positive payout (zeroed out when reinvested).
        if total_payout > 0 {
            let payment_token = Self::get_locked_payment_token_for_offering(&env, &offering_id)
                .ok_or(RevoraError::PaymentTokenMismatch)?;
            let contract_addr = env.current_contract_address();
        ...
        // Versioned v2 event: [2, holder, total_payout, periods] -- always emitted.
        Self::emit_v2_event(
            &env,
            (
                EVENT_CLAIM_V2,
...
impl RevoraRevenueShare {
    ...
    /// Configure dividend auto-reinvestment for a holder on an offering.
    ///
    /// When `enabled` is `true`, the next `claim` call will convert the holder's
    /// owed dividend into additional basis-point shares (at the configured NAV)
    /// instead of transferring payment tokens.
    ///
    /// ### Auth
    /// Requires `holder.require_auth()`. Only the holder can opt in or out.
    ///
    /// ### Parameters
    /// - `holder` — The holder configuring auto-reinvestment.
    /// - `issuer` / `namespace` / `token` — The offering to configure.
    /// - `enabled` — `true` to enable, `false` to disable.
    /// - `nav_per_share_e7` — NAV per basis-point share in 7-decimal payment-token
    ///   units.  Must be `> 0` when `enabled` is `true`; ignored when `false`.
    ///
    /// ### Errors
    /// - `ContractFrozen` — contract is halted.
    /// - `OfferingNotFound` — the offering does not exist.
    /// - `AutoReinvestInvalidNav` — `nav_per_share_e7 <= 0` while `enabled = true`.
    pub fn set_auto_reinvest(
        ...
        if enabled && nav_per_share_e7 <= 0 {
            return Err(RevoraError::AutoReinvestInvalidNav);
        }
        ...
        let cfg = AutoReinvestConfig { enabled, nav_per_share_e7 };
        env.storage().persistent().set(&DataKey2::AutoReinvest(offering_id, holder.clone()), &cfg);

        env.events().publish(
            (EVENT_DIVIDEND_REINVEST, issuer, namespace, token),
            (holder, enabled, nav_per_share_e7),
        );

        Ok(())
    }

    /// Read a holder's auto-reinvestment configuration for an offering.
    ///
    /// Returns `None` when the holder has never configured auto-reinvestment
    /// (equivalent to disabled with NAV 0).
    pub fn get_auto_reinvest(
        env: Env,
        issuer: Address,
        namespace: Symbol,
        token: Address,
        holder: Address,
    ) -> Option<AutoReinvestConfig> {
        let offering_id = OfferingId { issuer, namespace, token };
        env.storage().persistent().get(&DataKey2::AutoReinvest(offering_id, holder))
    }
}

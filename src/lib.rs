...
use soroban_sdk::{
  ...
  symbol_short, token, xdr::ToXdr, Address, Bytes, BytesN, Env, IntoVal, Map,
  Symbol, Vec,
};
...
const EVENT_REVENUE_REPORT_MISSING_OVERRIDE: Symbol =
  symbol_short!("rev_omiss");
...
pub const EVENT_PROOF_REJECT_DEPTH: Symbol =
  symbol_short!("proof_reject_depth");
...
impl AmountValidationMatrix {
  ...
  pub fn validate_snapshot_monotonic(
    new_ref: i128,
    last_ref: i128,
  ) -> Result<(), RevoraError> {
    if new_ref <= last_ref {
  ...
  pub fn category_for_function(
    fn_name: &str,
  ) -> Option<AmountValidationCategory> {
    match fn_name {
      ...
      "set_min_revenue_threshold" => {
        Some(AmountValidationCategory::MinRevenueThreshold)
      }
      "set_investment_constraints" => {
        Some(AmountValidationCategory::InvestmentMinStake)
      }
      "simulate_distribution" => Some(AmountValidationCategory::Simulation),
      "set_max_total_supply_shares" => {
        Some(AmountValidationCategory::MaxTotalSupplyShares)
      }
      _ => None,
    }
  }
...
impl RevoraRevenueShare {
    ...
    if let Some(stored_v) = env.storage().persistent().get::<DataKey, u32>(&key)
    {
      ...
      if stored_v < STORAGE_LAYOUT_VERSION {
        env
          .storage()
          .persistent()
          .set(&key, &STORAGE_LAYOUT_VERSION);
        env
          .events()
          .publish((EVENT_LAYOUT_VERSION,), STORAGE_LAYOUT_VERSION);
      }
    } else {
  ...
  fn require_not_offering_frozen(
    env: &Env,
  ...
  fn enforce_dispute_window(
    env: &Env,
    offering_id: &OfferingId,
  ) -> Result<(), RevoraError> {
    // If no period has been closed yet, there is no deadline to enforce.
    let last_closed = env.storage().persistent().get::<DataKey2, u64>(
      &DataKey2::LastClosedPeriodTimestamp(offering_id.clone()),
    );
  ...
  fn update_and_emit_reg_limit_delta(
    ...
    let agg_key =
      DataKey2::JurisdictionAggregateShare(offering_id.clone(), jur.clone());
    let old_aggregate: i128 =
      env.storage().persistent().get(&agg_key).unwrap_or(0);
  ...
  where
    ...
    T: IntoVal<Env, soroban_sdk::Val>
      + soroban_sdk::TryIntoVal<Env, soroban_sdk::Val>,
  {
    env
      .events()
      .publish(topic_tuple, (EVENT_SCHEMA_VERSION_V2, data));
  }
      ...
      + Clone,
  {
    env
      .events()
      .publish((EVENT_INDEXED_V2, topic_v2), data.clone());
    ...
    T: IntoVal<Env, soroban_sdk::Val> + Clone,
  {
    ...
    env
      .events()
      .publish((EVENT_INDEXED_V3, v3_topic), data.clone());
  ...
  fn require_report_window_open(
    ...
    if let Some(window) = env
      .storage()
      .persistent()
      .get::<WindowDataKey, AccessWindow>(&key)
    {
  ...
  fn set_holder_share_internal(
    ...
    if let Some(n) = nonce {
      let nonce_key =
        DataKey2::HolderShareNonce(offering_id.clone(), holder.clone());
      let last_nonce: u64 =
    ...
    let max_shares: i128 =
      env.storage().persistent().get(&max_shares_key).unwrap_or(0);
    if max_shares > 0 {
    ...
    if let Some(ref sc) = share_class {
      ...
      let current_class_shares: i128 = env
        .storage()
        .persistent()
        .get(&class_shares_key)
        .unwrap_or(0);
      ...
      env
        .storage()
        .persistent()
        .set(&class_shares_key, &new_class_shares);
    }

    // Persist updated holder share and running total.
    env.storage().persistent().set(
      &DataKey::HolderShare(offering_id.clone(), holder.clone()),
      &share_bps,
    );
    ...
    Self::update_and_emit_reg_limit_delta(
      env,
      &offering_id,
      &holder,
      delta_bps,
  ...
  fn get_holder_share_schedule(
    ...
    if current_share > 0 {
      schedule.push_back(HolderShareCheckpoint {
        start_index: 0,
        share_bps: current_share,
      });
    }
    schedule
  }
  ...
  fn get_checkpoint_threshold(env: &Env, offering_id: &OfferingId) -> u32 {
    ...
    env
      ...
      .persistent()
      .get(&key)
      .unwrap_or(CHECKPOINT_THRESHOLD_DEFAULT)
  }
  ...
  fn compute_anchor_claimable_sum(
    ...
    while current_index < end_idx {
      ...
      if current_share > 0 {
        ...
        let acc_start =
          Self::get_acc_per_share_at_index(env, offering_id, current_index);
  ...
  fn record_holder_share_transition(
    ...
    updated.push_back(HolderShareCheckpoint {
  ...
  fn get_holder_accrual_state(
    ...
    let last_claimed_idx: u32 = env
      .storage()
      .persistent()
      .get::<_, u32>(&DataKey::LastClaimedIdx(
        offering_id.clone(),
        holder.clone(),
      ))
      .unwrap_or(0);
  ...
  fn find_matured_claim_end_idx(
    ...
    while end_idx < period_count {
      ...
      let period_id: u64 =
        env.storage().persistent().get(&entry_key).unwrap_or(0);
      if period_id == 0 {
      ...
      let deposit_time: u64 =
        env.storage().persistent().get(&time_key).unwrap_or(0);
  ...
  fn cache_holder_accrual_through_matured(
    env: &Env,
    offering_id: &OfferingId,
    holder: &Address,
  ) {
    let mut state = Self::get_holder_accrual_state(env, offering_id, holder);
    let matured_end = Self::find_matured_claim_end_idx(
      env,
      offering_id,
    ...
    let anchor_key =
      DataKey2::AccrualAnchor(offering_id.clone(), holder.clone());
    let anchor: Option<AccrualAnchor> =
      env.storage().persistent().get(&anchor_key);
    ...
    if let Some(a) = anchor {
      if state.last_settled_idx <= a.end_idx {
        ...
        let anchor_end_incl =
          core::cmp::min(matured_end, a.end_idx.saturating_add(1));
        if anchor_start < anchor_end_incl {
          delta = delta.saturating_add(a.claimable_sum);
        }
        schedule_start =
          core::cmp::max(schedule_start, a.end_idx.saturating_add(1));
  ...
  fn get_allowed_jurisdictions_internal(
    env: &Env,
    offering_id: &OfferingId,
  ) -> Vec<Symbol> {
    env
  ...
  fn get_holder_jurisdiction_internal(
    ...
    env
      .storage()
      .persistent()
      .get(&DataKey2::HolderJurisdiction(
        offering_id.clone(),
        holder.clone(),
      ))
  }
  ...
  fn get_jurisdiction_grace_secs(env: &Env, offering_id: &OfferingId) -> u64 {
    env
      .storage()
      .persistent()
      .get::<DataKey2, u64>(&DataKey2::JurisdictionGracePeriod(
        offering_id.clone(),
      ))
  ...
  fn do_deposit_revenue(
    ...
    if let Err((err, reason)) = AmountValidationMatrix::validate(
      amount,
      AmountValidationCategory::RevenueDeposit,
    ) {
      env.events().publish(
        (
          EVENT_AMOUNT_VALIDATION_FAILED,
          issuer.clone(),
          namespace.clone(),
          token.clone(),
        ),
    ...
    if cap > 0 {
      ...
      let deposited: i128 =
        env.storage().persistent().get(&deposited_key).unwrap_or(0);
    ...
    let current_acc: i128 =
      env.storage().persistent().get(&global_acc_key).unwrap_or(0);
    ...
    if total_share_bps > 0 {
      ...
      let current_accrual: i128 = env
        .storage()
        .persistent()
        .get(&current_accrual_key)
        .unwrap_or(0);
      let new_accrual = current_accrual
        .checked_add(accrual_delta)
        .unwrap_or(current_accrual);
      env
        .storage()
        .persistent()
        .set(&current_accrual_key, &new_accrual);
    }
  ...
  pub fn get_supply_cap(
    ...
    env
      .storage()
      .persistent()
      .get(&DataKey2::SupplyCap(offering_id))
      .unwrap_or(0)
  }
  ...
  pub fn set_max_total_supply_shares(
    ...
    if max_total_supply_shares > 0 {
      env
        .storage()
        .persistent()
        .set(&max_shares_key, &max_total_supply_shares);
    } else {
  ...
  pub fn get_max_total_supply_shares(
    ...
    env
      .storage()
      .persistent()
      .get(&DataKey2::MaxTotalSupplyShares(offering_id))
      .unwrap_or(0)
  }
  ...
  pub fn get_total_shares_issued(
    ...
    env
      .storage()
      .persistent()
      .get(&DataKey2::TotalSharesIssued(offering_id))
      .unwrap_or(0)
  }
  ...
  pub fn set_offering_fee_bps(
    ...
    env.storage().persistent().set(
      &DataKey::OfferingFeeBps(offering_id, asset.clone()),
      &fee_bps,
    );
    env
      .events()
  ...
  pub fn set_secondary_market_royalty_bps(
    ...
    env.storage().persistent().set(
      &DataKey::OfferingRoyaltyBps(offering_id, asset.clone()),
      &royalty_bps,
    );
    env.events().publish(
  ...
  pub fn get_platform_fee_per_asset(env: Env, asset: Address) -> u32 {
    env
      .storage()
      .persistent()
      .get(&DataKey::PlatformFeePerAsset(asset))
      .unwrap_or(0)
  }
  ...
  pub fn set_offering_platform_fee(
    ...
    if !env
      .storage()
      .persistent()
      .has(&DataKey::OfferingIssuer(offering_id.clone()))
    {
      return Err(RevoraError::OfferingNotFound);
    }
    ...
    env.events().publish(
      (EVENT_PLAT_FEE_SET, issuer, namespace, token),
      (fee_bps, treasury),
    );
    Ok(())
  }
  ...
  pub fn get_offering_platform_fee(
    ...
    env
      .storage()
      .persistent()
      .get(&DataKey2::OfferingPlatformFee(offering_id))
  }
  ...
  fn get_min_revenue_threshold_for_offering(
    env: &Env,
    offering_id: &OfferingId,
  ) -> i128 {
    env
  ...
  fn ensure_issuer_registered(env: &Env, issuer: &Address) {
    ...
    if !env.storage().persistent().has(&issuer_key) {
      let count: u32 = env
        .storage()
        .persistent()
        .get(&DataKey2::IssuerCount)
        .unwrap_or(0);
      env
        .storage()
        .persistent()
        .set(&DataKey2::IssuerItem(count), issuer);
      env
        .storage()
        .persistent()
        .set(&DataKey2::IssuerCount, &(count + 1));
      env.storage().persistent().set(&issuer_key, &true);
    }
  }

  fn ensure_namespace_registered(
    env: &Env,
    issuer: &Address,
    namespace: &Symbol,
  ) {
    let ns_key =
      DataKey2::NamespaceRegistered(issuer.clone(), namespace.clone());
    if !env.storage().persistent().has(&ns_key) {
      let ns_count: u32 = env
        ...
        .unwrap_or(0);
      env.storage().persistent().set(
        &DataKey2::NamespaceItem(issuer.clone(), ns_count),
        namespace,
      );
      env
        .storage()
  ...
  pub fn set_testnet_mode(env: Env, enabled: bool) -> Result<(), RevoraError> {
    ...
    env
      .storage()
      .persistent()
      .set(&DataKey::TestnetMode, &enabled);
  ...
  pub fn set_storage_layout_version(
    ...
    env
      .storage()
      .persistent()
      .set(&DataKey::StorageLayoutVersion, &v);
  ...
  pub fn get_pending_issuer_transfer(
    ...
    env
      .storage()
      .persistent()
      .get::<DataKey, PendingTransfer>(&DataKey::PendingIssuerTransfer(
        offering_id,
      ))
  ...
  pub fn get_pending_transfer_details(
    ...
    env.storage().persistent().get::<DataKey, PendingTransfer>(
      &DataKey::PendingIssuerTransfer(offering_id),
    )
  }
  ...
  pub fn propose_issuer_transfer(
    ...
    Self::do_propose_issuer_transfer(
      env, issuer, namespace, token, new_issuer, 0,
    )
  }
  ...
  fn do_propose_issuer_transfer(
    ...
    env.events().publish(
      (
        EVENT_ISSUER_TRANSFER_PROPOSED,
        issuer.clone(),
        namespace.clone(),
        token.clone(),
      ),
  ...
  pub fn accept_issuer_transfer(
    ...
    let offering_id = Self::find_pending_transfer_for_new_issuer(
      &env,
      &namespace,
      &token,
      &new_issuer,
    ...
    Self::ensure_namespace_registered(
      &env,
      &new_issuer,
    ...
    env.storage().persistent().set(
      &DataKey2::OfferingRecord(new_offering_id.clone()),
      &offering,
    );

    // Update issuer lookups for the old and new offering IDs.
    env.storage().persistent().set(
      &DataKey::OfferingIssuer(offering_id.clone()),
      &new_issuer.clone(),
    );
    env.storage().persistent().set(
      &DataKey::OfferingIssuer(new_offering_id.clone()),
      &new_issuer.clone(),
    );
    ...
    if let Some(config) = env
      .storage()
      .persistent()
      .get::<_, ConcentrationLimitConfig>(&DataKey::ConcentrationLimit(
        offering_id.clone(),
      ))
    {
      env.storage().persistent().set(
        &DataKey::ConcentrationLimit(new_offering_id.clone()),
        &config,
      );
      env
        .storage()
        .persistent()
        .remove(&DataKey::ConcentrationLimit(offering_id.clone()));
    }
    ...
    {
      env.storage().persistent().set(
        &DataKey::CurrentConcentration(new_offering_id.clone()),
        &current,
      );
      env
        .storage()
        .persistent()
        .remove(&DataKey::CurrentConcentration(offering_id.clone()));
    }
      ...
      .get::<_, RoundingMode>(&DataKey::RoundingMode(offering_id.clone()))
    {
      env
        .storage()
        .persistent()
        .set(&DataKey::RoundingMode(new_offering_id.clone()), &mode);
      env
        .storage()
        .persistent()
        .remove(&DataKey::RoundingMode(offering_id.clone()));
    }
    if let Some(constraints) = env
      .storage()
      .persistent()
      .get::<_, InvestmentConstraintsConfig>(
      &DataKey2::InvestmentConstraints(offering_id.clone()),
    ) {
      env.storage().persistent().set(
        &DataKey2::InvestmentConstraints(new_offering_id.clone()),
        &constraints,
      );
      env
        .storage()
      ...
      .get::<_, u64>(&DataKey::ClaimDelaySecs(offering_id.clone()))
    {
      ...
      env
        .storage()
        .persistent()
        .remove(&DataKey::ClaimDelaySecs(offering_id.clone()));
    }
    if let Some(snap_config) = env
      .storage()
      .persistent()
      .get::<_, bool>(&DataKey::SnapshotConfig(offering_id.clone()))
    {
      env.storage().persistent().set(
        &DataKey::SnapshotConfig(new_offering_id.clone()),
        &snap_config,
      );
      env
        .storage()
        .persistent()
        .remove(&DataKey::SnapshotConfig(offering_id.clone()));
    }
    if let Some(snap_ref) = env
      .storage()
      .persistent()
      .get::<_, u64>(&DataKey::LastSnapshotRef(offering_id.clone()))
    {
      env.storage().persistent().set(
        &DataKey::LastSnapshotRef(new_offering_id.clone()),
        &snap_ref,
      );
      env
        .storage()
        .persistent()
        .remove(&DataKey::LastSnapshotRef(offering_id.clone()));
    }
  ...
  pub fn initialize(
    ...
    env
      .storage()
      .persistent()
      .set(&DataKey::Paused, &PauseState::NotPaused);
    let eo = event_only.unwrap_or(false);
    env
      .storage()
      .persistent()
      .set(&DataKey2::ContractFlags, &(false, eo));
    ...
    env
      .storage()
      .persistent()
      .set(&DataKey::DeployedVersion, &CONTRACT_VERSION);

    env
      .events()
      .publish((EVENT_INIT, admin.clone()), (safety, eo));
  }
  ...
  pub fn pause_admin(env: Env, caller: Address) -> Result<(), RevoraError> {
    ...
    env
      .storage()
      .persistent()
      .set(&DataKey::Paused, &PauseState::SoftPaused);
    // Legacy compatibility event
    env.events().publish((EVENT_PAUSED, caller.clone()), ());
    // Versioned tier event
    env
      .events()
      .publish((EVENT_PAUSED2, caller.clone()), (PauseState::SoftPaused,));
    Ok(())
  }
  ...
  pub fn unpause_admin(env: Env, caller: Address) -> Result<(), RevoraError> {
    ...
    env
      .events()
      .publish((EVENT_PAUSED2, caller.clone()), (PauseState::NotPaused,));
    Ok(())
  }
  ...
  pub fn hard_pause_admin(
    ...
    env
      .storage()
      .persistent()
      .set(&DataKey::Paused, &PauseState::HardPaused);
    env.events().publish((EVENT_PAUSED, caller.clone()), ());
    env
      .events()
      .publish((EVENT_PAUSED2, caller.clone()), (PauseState::HardPaused,));
    Ok(())
  }
  ...
  pub fn set_emit_v2_compat(
    ...
    env
      .storage()
      .persistent()
      .set(&DataKey2::EmitV2Compat, &enabled);
  ...
  pub fn register_offering(
    ...
    if let Some(ref cls_vec) = classes {
      ...
      for (_, config) in cls_vec.iter() {
        sum_bps = sum_bps
          .checked_add(config.bps)
          .ok_or(RevoraError::InvalidShareClassBps)?;
      }
      if sum_bps != 10_000 {
    ...
    let ns_reg_key =
      DataKey2::NamespaceRegistered(primary_issuer.clone(), namespace.clone());
    if !env.storage().persistent().has(&ns_reg_key) {
      let ns_count_key = DataKey2::NamespaceCount(primary_issuer.clone());
      let count: u32 =
        env.storage().persistent().get(&ns_count_key).unwrap_or(0);
      env.storage().persistent().set(
        &DataKey2::NamespaceItem(primary_issuer.clone(), count),
        &namespace,
      );
    ...
    env
      .storage()
      .persistent()
      .set(&issuer_lookup_key, &primary_issuer);
  ...
  pub fn get_denomination_metadata(
    ...
    env.storage().persistent().get::<DataKey2, (Symbol, u32)>(
      &DataKey2::DenominationMetadata(offering_id),
    )
  }
  ...
  pub fn get_offering(
    ...
    let count =
      Self::get_offering_count(env.clone(), issuer.clone(), namespace.clone());
    let tenant_id = TenantId { issuer, namespace };
    for i in 0..count {
      let item_key = DataKey::OfferItem(tenant_id.clone(), i);
      let offering: Offering =
        env.storage().persistent().get(&item_key).unwrap();
  ...
  pub fn set_oracle_chain(
    ...
    env
      .storage()
      .persistent()
      .set(&DataKey2::OracleChain(offering_id), &chain);
    Ok(())
  }
  ...
  pub fn get_oracle_chain(
    ...
    env
      .storage()
      .persistent()
      .get::<DataKey2, OracleChain>(&DataKey2::OracleChain(offering_id))
  }
  ...
  pub fn register_oracle_pubkey(
    ...
    env
      .storage()
      .persistent()
      .set(&DataKey2::OraclePubKey(oracle_id), &pubkey);
    Ok(())
  }
  ...
  fn try_oracle_entry(
    ...
    if entry.max_age_secs > 0
      && now.saturating_sub(quoted_at) > entry.max_age_secs
    {
  ...
  fn convert_report_amount_if_needed(
      ...
      for entry in chain.entries.iter() {
        match Self::try_oracle_entry(env, &entry, now) {
          Ok(rate) => {
            ...
            let converted =
              amount.saturating_mul(rate).saturating_div(BPS_DENOMINATOR);
    ...
    let (rate, quoted_at) =
      if let (Some(q), Some(sig)) = (quote_bytes, signature) {
  ...
  fn report_revenue_internal(
    ...
    let last_report_period_key =
      DataKey2::LastReportedPeriodId(offering_id.clone());
    let current_timestamp = env.ledger().timestamp();
      ...
      if !testnet_mode {
          ...
          .get::<DataKey, ConcentrationLimitConfig>(&limit_key)
        {
          ...
          if config.enforce && config.max_bps > 0 {
            ...
            let current: u32 =
              env.storage().persistent().get(&curr_key).unwrap_or(0);
    ...
    let threshold =
      Self::get_min_revenue_threshold_for_offering(&env, &offering_id);
    ...
    if event_only {
      ...
      match reports.get(period_id) {
        Some((existing_amount, _)) => {
          ...
          let closed_key =
            DataKey2::ClosedPeriod(offering_id.clone(), period_id);
          ...
          let mut summary: AuditSummary =
            env.storage().persistent().get(&summary_key).unwrap_or(
              AuditSummary {
                total_revenue: 0,
                report_count: 0,
              },
          ...
          summary.total_revenue =
            summary.total_revenue.s_add(delta).unwrap_or(i128::MAX);
        ...
        None => {
          ...
          Self::require_next_period_id(
            &env,
            last_report_period_key.clone(),
            period_id,
          )?;
          if threshold > 0 && amount < threshold {
            env.events().publish(
          ...
          summary.report_count =
            summary.report_count.s_add(1).unwrap_or(u64::MAX);
          env.storage().persistent().set(&summary_key, &summary);
    ...
    if !event_only {
      Self::update_and_emit_accrual_index(
        &env,
        &offering_id,
        amount,
        period_id,
  ...
  pub fn repair_audit_summary(
    ...
    .ok_or(RevoraError::OfferingNotFound)?;
    let admin =
      Self::get_admin(env.clone()).ok_or(RevoraError::NotInitialized)?;
    ...
    let (corrected, _) =
      Self::compute_audit_summary_from_reports(&env, &offering_id);
  ...
  fn do_blacklist_add(
    ...
    if !Self::is_event_only(&env) {
      ...
      let mut map: Map<Address, SanctionsAttestation> = env
        .storage()
        .persistent()
        .get(&key)
        .unwrap_or_else(|| Map::new(&env));
  ...
  pub fn blacklist_remove(
    ...
    env
      .events()
      .publish((EVENT_BL_REM, issuer, namespace, token), (caller, investor));
    Ok(())
  }
  ...
  pub fn estimate_transfer(
    ...
    if let Some(schedule) = Self::get_lockup_schedule(
      env.clone(),
      issuer.clone(),
      namespace.clone(),
      token.clone(),
      ...
      if existing != category {
        if to_share > 0 {
          ...
          env
            .storage()
            .persistent()
            .set(&old_count_key, &old_count.saturating_sub(1));

          let new_count_key = DataKey2::CategoryHolderCount(
            offering_id.clone(),
            category.clone(),
          );
          let new_count: u32 =
    ...
    let jurisdiction =
      Self::get_holder_jurisdiction_internal(&env, &offering_id, &from);
    if let Some(jur) = jurisdiction {
      if jur != EVENT_JUR_UNSET {
        let cooldown_key =
          DataKey2::TransferCooldownConfig(offering_id.clone(), jur.clone());
        if let Some(cooldown_secs) = env
          .storage()
          .persistent()
          .get::<DataKey2, u64>(&cooldown_key)
        {
          if cooldown_secs > 0 {
            let last_xfer_key = DataKey2::HolderLastTransferTime(
              offering_id.clone(),
              from.clone(),
            );
            let last_xfer: u64 =
  ...
  pub fn transfer_with_attestation(
    ...
    if let Some(j) = jur {
      if j != EVENT_JUR_UNSET {
        let cooldown_key =
          DataKey2::TransferCooldownConfig(offering_id.clone(), j.clone());
  ...
  pub fn set_rounding_mode(
    ...
    Self::emit_v2_event(
      &env,
      (EVENT_ROUNDING_MODE_SET, issuer, namespace, token),
      mode,
    );
    Ok(())
  }
  ...
  pub fn set_investment_constraints(
    ...
    let previous = env
      .storage()
      .persistent()
      .get::<DataKey2, InvestmentConstraintsConfig>(&key);
    env.storage().persistent().set(
      &key,
      &InvestmentConstraintsConfig {
        min_stake,
        max_stake,
      },
    );
  ...
  pub fn compute_share(
    ...
    let share = base.s_add(remainder_share).unwrap_or_else(|_| {
      if (base >= 0 && remainder_share >= 0)
        || (base < 0 && remainder_share < 0)
      {
        if base >= 0 {
          i128::MAX
        } else {
  ...
  pub fn set_payment_token_decimals(
    ...
    env
      .storage()
      .persistent()
      .set(&DataKey2::PaymentTokenDecimals(offering_id), &decimals);
    env
      .events()
  ...
  pub fn commit_snapshot(
    ...
    let last_ref: u64 =
      env.storage().persistent().get(&last_ref_key).unwrap_or(0);
  ...
  pub fn apply_snapshot_shares(
    ...
    let mut slot_count: u32 = env
      .storage()
      .persistent()
      .get(&DataKey::SnapshotHolderCount(
        offering_id.clone(),
        snapshot_ref,
      ))
      .unwrap_or(0);
    ...
    if max_shares > 0 {
      env.storage().persistent().set(
        &DataKey2::TotalSharesIssued(offering_id.clone()),
        &temp_total_shares,
      );
    } else {
    ...
    env.storage().persistent().set(
      &DataKey::SnapshotHolderCount(offering_id.clone(), snapshot_ref),
      &slot_count,
    );

    // Persist updated per-offering running total.
    env.storage().persistent().set(
      &DataKey::HolderShareTotal(offering_id.clone()),
      &current_total,
    );
  ...
  pub fn get_snapshot_holder_at(
    ...
    env.storage().persistent().get(&DataKey::SnapshotHolder(
      offering_id,
      snapshot_ref,
      index,
    ))
  ...
  pub fn open_dispute(
    ...
    if env
      .storage()
      .persistent()
      .has(&DataKey2::Dispute(dispute_id.clone()))
    {
    ...
    env
      .storage()
      .persistent()
      .set(&DataKey2::Dispute(dispute_id.clone()), &dispute);
    env.storage().persistent().set(&count_key, &(count + 1));
  ...
  pub fn get_dispute(env: Env, dispute_id: BytesN<32>) -> Option<Dispute> {
    env
      .storage()
      .persistent()
      .get(&DataKey2::Dispute(dispute_id))
  }
  ...
  pub fn get_holder_share(
      ...
      .get::<_, Vec<(ShareClass, ClassConfig)>>(&classes_key)
    {
      ...
      env
        .storage()
        .persistent()
        .get(&DataKey::HolderShare(offering_id, holder))
        .unwrap_or(0)
    }
  }
  ...
  pub fn convert_class(
    ...
    if let Some(schedule) = env
      .storage()
      .persistent()
      .get::<_, crate::vesting::VestingSchedule>(
      &crate::vesting::VestingKey::Schedule(holder.clone()),
    ) {
      let vested = crate::vesting::VestingContract::get_vested_amount(
        env.clone(),
        holder.clone(),
      )
      .unwrap_or(0);
    ...
    let ratio_bps: u32 = env
      ...
      .persistent()
      .get(&ratio_key)
      .ok_or(RevoraError::ConversionNotApproved)?;

    if ratio_bps == 0 {
      return Err(RevoraError::InvalidConversionRatio);
    }

    let from_key = DataKey2::HolderShareClass(
      offering_id.clone(),
      holder.clone(),
      from_class.clone(),
    );
    let to_key = DataKey2::HolderShareClass(
      offering_id.clone(),
      holder.clone(),
      to_class.clone(),
    );

    let from_balance: u32 =
      env.storage().persistent().get(&from_key).unwrap_or(0);
      ...
      .get::<_, Vec<(ShareClass, ClassConfig)>>(&classes_key)
    {
      ...
      if let (Some(f_idx), Some(t_idx)) = (from_idx, to_idx) {
        ...
        f_cfg.bps = f_cfg
          ...
          .ok_or(RevoraError::InvalidShareBps)?;
        t_cfg.bps = t_cfg
          .bps
  ...
  pub fn set_transfer_cooldown(
    ...
    let key =
      DataKey2::TransferCooldownConfig(offering_id, jurisdiction.clone());
  ...
  pub fn set_claim_delay(
    ...
    env
      .storage()
      .persistent()
      .set(&DataKey::ClaimDelaySecs(offering_id), &delay_secs);
    Ok(())
  }
  ...
  pub fn get_claim_delay(
    ...
    env
      .storage()
      .persistent()
      .get(&DataKey::ClaimDelaySecs(offering_id))
      .unwrap_or(0)
  }
  ...
  pub fn set_jurisdiction_grace_period(
    ...
    if grace_secs < MIN_JURISDICTION_GRACE_SECS
      || grace_secs > MAX_JURISDICTION_GRACE_SECS
    {
      return Err(RevoraError::InvalidAmount);
    }
  ...
  pub fn get_jurisdiction_migration(
    ...
    env
      .storage()
      .persistent()
      .get::<DataKey2, JurisdictionMigrationState>(
  ...
  pub fn migrate_storage(
    ...
    env
      .storage()
      .persistent()
      .set(&DataKey::DeployedVersion, &to);
    env
      .events()
      .publish((symbol_short!("migrate"),), (from, to));
    Ok(())
  }
  ...
  pub fn set_redemption_window(
    ...
    let existing: Option<AccessWindow> = env
      .storage()
      .persistent()
      .get(&WindowDataKey::Redemption(offering_id.clone()));
    if let Some(existing) = existing {
      if start_timestamp < existing.end_timestamp
        && existing.start_timestamp < end_timestamp
      {
    ...
    env
      .storage()
      .persistent()
      .set(&WindowDataKey::Redemption(offering_id), &new_window);
    env.events().publish(
  ...
  pub fn get_redemption_window(
    ...
    env
      .storage()
      .persistent()
      .get(&WindowDataKey::Redemption(offering_id))
  }
  pub fn claim(
    env: Env,
    holder: Address,
    ...
    if let Some(a) = env
      .storage()
      .persistent()
      .get::<DataKey2, AccrualAnchor>(&anchor_key)
    {
  ...
  pub fn open_dispute(
    ...
    if severity == DisputeSeverity::Critical {
      ...
      let crit_count: u32 =
        env.storage().persistent().get(&crit_key).unwrap_or(0);
  ...
  pub fn resolve_dispute(
    ...
    env
      .storage()
      .persistent()
      .set(&DataKey2::Dispute(dispute_id), &dispute);

    // Decrement critical dispute count and emit freeze_off when it hits zero
    if was_critical {
      let crit_key =
        DataKey2::CriticalDisputeCount(dispute.offering_id.clone());
      let crit_count: u32 =
        env.storage().persistent().get(&crit_key).unwrap_or(0);
      if crit_count > 0 {
...
impl RevoraRevenueShare {
  ...
  pub fn claim(
    ...
    if let Some(a2) = env
      .storage()
      .persistent()
      .get::<DataKey2, AccrualAnchor>(&anchor_key2)
    {
  ...
  pub fn close_period(
    ...
    env.storage().persistent().set(
      &DataKey2::LastClosedPeriodTimestamp(offering_id),
      &closed_at,
    );
  ...
  pub fn is_period_closed(
    ...
    env
      .storage()
      .persistent()
      .has(&DataKey2::ClosedPeriod(offering_id, period_id))
  }
  ...
  fn compute_period_close_preview(
    ...
    let n = holders.len();
    let mut payout_rows: std::vec::Vec<(u32, u32, Address, i128)> =
      std::vec::Vec::new();
    let mut total: i128 = 0;
  ...
  pub fn get_class_pay_order(
    ...
    env
      .storage()
      .persistent()
      .get::<DataKey2, Vec<ShareClass>>(&DataKey2::ClassPayOrder(
        offering_id,
        period_id,
      ))
      .unwrap_or_else(|| Vec::new(&env))
  ...
  pub fn set_dual_sig_config(
    ...
    env
      .storage()
      .persistent()
      .set(&DataKey2::DualSigEnabled(offering_id), &enabled);
  ...
  pub fn get_disclosure(
    ...
    env
      .storage()
      .persistent()
      .get(&DataKey2::DisclosureMeta(offering_id))
  }
}
...
impl RevoraRevenueShare {
  ...
  pub fn set_meta_delegate(
    ...
    Self::emit_v2_event(
      &env,
      (EVENT_META_DELEGATE_SET, issuer, namespace, token),
      delegate,
    );
    Ok(())
  }
  ...
  pub fn meta_set_holder_share(
    ...
    Self::verify_meta_signature(
      &env, &signer, nonce, expiry, action, &signature,
    )?;
    Self::set_holder_share_internal(
      &env,
      payload.issuer.clone(),
  ...
  fn compute_claimable_preview(
    ...
    if let Some(a) = anchor {
      if holder_start_idx <= a.end_idx {
        ...
        let anchor_periods =
          a.end_idx.saturating_sub(holder_start_idx).saturating_add(1);
    ...
    let effective_end = count
      .map(|c| core::cmp::min(actual_start + c, period_count))
  ...
  pub fn request_redemption(
    ...
    let request_key =
      DataKey2::RedemptionRequest(offering_id.clone(), holder.clone());
    ...
    // Emit event
    env.events().publish(
      (EVENT_REDEMPTION_REQUESTED, issuer, namespace, token),
      (holder, shares_bps),
    );
    Ok(())
  }
  ...
  pub fn fulfill_redemption(
    ...
    let request_key =
      DataKey2::RedemptionRequest(offering_id.clone(), holder.clone());
    let pending: PendingRedemption = env
      .storage()
      .persistent()
      .get(&request_key)
      .ok_or(RevoraError::NoTransferPending)?;
    ...
    let (net_amount, fee_amount, treasury_addr) = if let Some(cfg) = fee_config
    {
      if cfg.fee_bps > 0 {
        let fee =
          amount.checked_mul(cfg.fee_bps as i128).unwrap_or(0) / 10_000i128;
    ...
    if net_amount > 0
      && token_client
        .try_transfer(&contract_addr, &holder, &net_amount)
        .is_err()
    {
      return Err(RevoraError::TransferFailed);
    }
    ...
    if fee_amount > 0 {
      if let Some(treasury) = treasury_addr {
        if token_client
          .try_transfer(&contract_addr, &treasury, &fee_amount)
          .is_err()
        {
          return Err(RevoraError::TransferFailed);
        }
  ...
  pub fn set_redemption_fee_bps(
    ...
    let config = RedemptionFeeConfig {
      fee_bps,
      treasury: treasury.clone(),
    };
    ...
    env.events().publish(
      (EVENT_REDEMPTION_FEE_SET, issuer, namespace, token),
      (fee_bps, treasury),
    );
    Ok(())
  }
  ...
  pub fn get_unlocked_bps(
    ...
    let schedule =
      Self::get_lockup_schedule(env.clone(), issuer, namespace, token);
    let now = env.ledger().timestamp();
    schedule
      .map(|s| s.calculate_unlocked_bps(now))
      .unwrap_or(10_000)
  }
  ...
  pub fn extend_lockup(
    ...
    let existing: LockupSchedule = env
      .storage()
      .persistent()
      .get(&key)
  ...
  pub fn get_claimable(
    ...
    let (total, _) =
      Self::compute_claimable_preview(&env, &offering_id, &holder, 0, None);
    total
  }
  ...
  pub fn get_holder_accrued_unclaimed(
    ...
    if let Some(anchor) = env
      .storage()
      .persistent()
      .get::<_, AccrualAnchor>(&anchor_key)
    {
      let last_claimed_idx: u32 = env
        .storage()
        .persistent()
        .get(&DataKey::LastClaimedIdx(
          offering_id.clone(),
          holder.clone(),
        ))
        .unwrap_or(0);
  ...
  pub fn set_fiscal_year_start(
    ...
    env
      .storage()
      .persistent()
      .set(&DataKey2::FiscalYearStartMonth(offering_id), &month);
    Ok(())
  }
  ...
  fn set_claim_delay_full(
    ...
    env.events().publish(
      (EVENT_CLAIM_DELAY_SET, issuer, namespace, token),
      delay_secs,
    );
    Ok(())
  }
...
impl RevoraRevenueShare {
  ...
  pub fn simulate_distribution(
    ...
    let classes: Option<Vec<(ShareClass, ClassConfig)>> =
      env.storage().persistent().get(&classes_key);
    let mode =
      Self::get_rounding_mode(env.clone(), issuer, namespace, token.clone());
  ...
  pub fn finalize_admin_rotation(
    env: Env,
    new_admin: Address,
  ) -> Result<(), RevoraError> {
    Self::require_not_frozen(&env)?;
    ...
    if delay > 0 {
      let elapsed =
        env.ledger().timestamp().saturating_sub(pending.proposed_at);
      if elapsed < delay {
    ...
    env
      .storage()
      .persistent()
      .set(&DataKey2::AdminRotationLog(rotation_id), &entry);
    env
      .storage()
      .persistent()
      .set(&DataKey2::AdminRotationCount, &rotation_id);
    ...
    if rotation_id > MAX_ADMIN_ROTATION_LOG {
      ...
      env
        .storage()
        .persistent()
        .remove(&DataKey2::AdminRotationLog(evict_id));
    }
  ...
  pub fn cancel_admin_rotation(env: Env) -> Result<(), RevoraError> {
    ...
    env
      .events()
      .publish((symbol_short!("adm_canc"), admin), pending.new_admin);
  ...
  pub fn revoke_admin_rotation(env: Env) -> Result<(), RevoraError> {
    ...
    env
      .events()
      .publish((EVENT_ADMIN_ROTATION_REVOKED, admin), pending.new_admin);
  ...
  /// ### Auth
  /// None â€” read-only.
  pub fn get_pending_admin_rotation_details(
    env: Env,
  ) -> Option<PendingAdminRotation> {
  ...
  pub fn get_admin_rotation_history_page(
    ...
    for i in start as u64..end {
      ...
      let entry: AdminRotationEntry =
        env.storage().persistent().get(&log_key).unwrap();
  ...
  pub fn freeze_offering(
    ...
    env.events().publish(
      (EVENT_FREEZE_OFFERING, issuer, namespace, token),
      (caller, true),
    );
    Ok(())
  }
  ...
  pub fn unfreeze_offering(
    ...
    env.events().publish(
      (EVENT_UNFREEZE_OFFERING, issuer, namespace, token),
      (caller, false),
    );
    Ok(())
  }
  ...
  pub fn emergency_unfreeze_holder(
    ...
    Self::clear_freeze_reason(
      env, caller, issuer, namespace, token, holder, reason,
    )
  }
  ...
  pub fn process_ofac_attestation(
    ...
    if !Self::is_event_only(&env) {
      ...
      let current_mask: u32 =
        env.storage().persistent().get(&fk).unwrap_or_else(|| {
          env
            .storage()
            .persistent()
            .get::<DataKey2, FreezeReason>(&legacy_key)
      ...
      env.storage().persistent().set(
        &fk,
        &(current_mask | FreezeReason::SanctionsMatch.to_bitmask()),
      );
    }
  ...
  pub fn init_multisig(
    ...
    env
      .storage()
      .persistent()
      .set(&DataKey2::MultisigThreshold, &threshold);
    env
      .storage()
      .persistent()
      .set(&DataKey2::MultisigOwners, &owners.clone());
    env
      .storage()
      .persistent()
      .set(&DataKey2::MultisigProposalCount, &0_u32);
    env
      .storage()
      .persistent()
      .set(&DataKey2::MultisigProposalDuration, &proposal_duration);
    env.events().publish(
      (EVENT_MULTISIG_INIT, caller.clone()),
  ...
  pub fn create_proposal(
    ...
    if env
      .storage()
      .persistent()
      .has(&DataKey2::GovernanceProposalMeta(
        offering_id.clone(),
        meta_hash.clone(),
      ))
    {
    ...
    env.storage().persistent().set(
      &DataKey2::GovernanceProposal(offering_id.clone(), proposal_id),
      &proposal,
    );
    env
      .storage()
      .persistent()
      .set(&count_key, &(proposal_id + 1));
    env.storage().persistent().set(
      &DataKey2::GovernanceProposalMeta(offering_id.clone(), meta_hash.clone()),
      &true,
    );
    env.events().publish(
      (
        EVENT_PROPOSAL_CREATED_GOV,
        issuer.clone(),
        namespace.clone(),
        token.clone(),
      ),
  ...
  pub fn get_proposal(
    ...
    env
      .storage()
      .persistent()
      .get(&DataKey2::GovernanceProposal(offering_id, proposal_id))
  }
  ...
  pub fn propose_action(
    ...
    let quorum_bps: u32 = env
      .storage()
      .persistent()
      .get(&DataKey2::MultisigQuorumBps)
      .unwrap_or(5100);
    ...
    env
      .storage()
      .persistent()
      .set(&DataKey2::MultisigProposal(id), &proposal);
    env.storage().persistent().set(&count_key, &(id + 1));

    env
      .events()
      .publish((EVENT_PROPOSAL_CREATED, proposer.clone()), (id, expiry));
    env
      .events()
      .publish((EVENT_PROPOSAL_APPROVED, proposer), id);
    Ok(id)
  }
  ...
  pub fn approve_action(
    ...
    env
      .events()
      .publish((EVENT_PROPOSAL_APPROVED, approver.clone()), proposal_id);
  ...
  pub fn execute_action(
    ...
    match proposal.action.clone() {
      ...
      ProposalAction::Freeze => {
        ...
        Self::emit_v2_event(
          &env,
          (EVENT_FREEZE_V2, proposal.proposer.clone()),
          true,
        );
      }
      ProposalAction::SetThreshold(new_threshold) => {
        ...
        env
          .storage()
          .persistent()
          .set(&DataKey2::MultisigThreshold, &new_threshold);
        let next_epoch = current_epoch + 1;
        env
          .storage()
          .persistent()
          .set(&DataKey2::MultisigEpoch, &next_epoch);
      }
      ProposalAction::AddOwner(new_owner) => {
        ...
        env
          .storage()
          .persistent()
          .set(&DataKey2::MultisigOwners, &owners);
        let next_epoch = current_epoch + 1;
        env
          .storage()
          .persistent()
      ...
      ProposalAction::RemoveOwner(old_owner) => {
        ...
        env
          .storage()
          .persistent()
          .set(&DataKey2::MultisigOwners, &new_owners);
        let next_epoch = current_epoch + 1;
        env
          .storage()
          .persistent()
      ...
      ProposalAction::SetProposalDuration(new_duration) => {
        ...
        env
          .storage()
          .persistent()
          .set(&DataKey2::MultisigProposalDuration, &new_duration);
        env.events().publish(
    ...
    env
      .events()
      .publish((EVENT_PROPOSAL_EXECUTED, executor), proposal_id);
    Ok(())
  }
  ...
  pub fn faucet_seed_holders(
    ...
    if let Some(last_ts) = last_request_ts {
      if now.saturating_sub(last_ts) < DEFAULT_FAUCET_COOLDOWN_SECONDS {
        ...
        let rejects: u32 = env
          ...
          .unwrap_or(0u32);
        env.storage().persistent().set(
          &DataKey2::FaucetMetricsCooldownRejects,
          &rejects.saturating_add(1),
        );
    ...
    let addr_seen_key =
      DataKey2::FaucetMetricsAddrSeen(current_window_id, requester.clone());
    if !env.storage().persistent().has(&addr_seen_key) {
      ...
      let unique: u32 = env
        ...
        .unwrap_or(0u32);
      env.storage().persistent().set(
        &DataKey2::FaucetMetricsUniqueAddrs,
    ...
    let dispensed: u32 = env
      ...
      .unwrap_or(0u32);
    env.storage().persistent().set(
      &DataKey2::FaucetMetricsTotalDispensed,
      &dispensed.saturating_add(count),
    );
    ...
    let prev_count: u32 = env
      ...
      .unwrap_or(0);
    if count > prev_count {
      env.storage().persistent().set::<DataKey2, u32>(
        &DataKey2::FaucetSeedCount(offering_id.clone()),
        &count,
      );
    }
...
mod issue_455_fx_oracle_tests {
  ...
  fn report_revenue_converts_cross_currency_amount_with_registered_oracle() {
    ...
    assert_eq!(
      client.get_revenue_by_period(&issuer, &namespace, &token, &1),
      1_200
    );
    assert_eq!(
      client
...
mod oracle_chain_tests {
  ...
  fn chain_first_oracle_fresh_uses_first_entry() {
    ...
    assert_eq!(
      client.get_revenue_by_period(&issuer, &ns, &token, &1),
      1_200
    );
    assert_eq!(
      client
        .get_audit_summary(&issuer, &ns, &token)
        .unwrap()
        .total_revenue,
      1_200
    );
...
mod issue_370_373_tests {
  ...
  fn issue_370_get_offerings_page_limit_cursor_and_order_are_stable() {
    ...
    let (page_1, cursor_1) =
      client.get_offerings_page(&issuer, &namespace, &0, &10);
    ...
    let (page_2, cursor_2) =
      client.get_offerings_page(&issuer, &namespace, &10, &10);
    ...
    let (page_3, cursor_3) =
      client.get_offerings_page(&issuer, &namespace, &20, &10);
  ...
  fn issue_370_get_offerings_page_stable_across_accept_issuer_transfer() {
    ...
    env.as_contract(&contract_id, || {
      env
        .storage()
        .persistent()
        .set(&DataKey2::IssuerCount, &1_u32);
      env
        .storage()
        .persistent()
        .set(&DataKey2::IssuerItem(0), &old_issuer);
      env
        .storage()
        .persistent()
      ...
      env.storage().persistent().set(
        &DataKey2::NamespaceRegistered(old_issuer.clone(), namespace.clone()),
        &true,
      );
    });
    ...
    let (old_page, old_cursor) =
      client.get_offerings_page(&old_issuer, &namespace, &0, &100);
    ...
    let (new_page_1, new_cursor_1) =
      client.get_offerings_page(&new_issuer, &namespace, &0, &2);
    ...
    let (new_page_2, new_cursor_2) =
      client.get_offerings_page(&new_issuer, &namespace, &2, &2);
  ...
  fn issue_373_compute_share_round_half_up_negative_midpoint_and_extremes() {
    ...
    assert_eq!(
      client.compute_share(&0, &5_000, &RoundingMode::RoundHalfUp),
      0
    );
    assert_eq!(
      client.compute_share(&123_456, &0, &RoundingMode::RoundHalfUp),
      0
    );
    assert_eq!(
      client.compute_share(&15_000, &5_000, &RoundingMode::RoundHalfUp),
      7_500
    );
    assert_eq!(
      client.compute_share(&-15_001, &5_000, &RoundingMode::Truncation),
      -7_500
    );
    assert_eq!(
      client.compute_share(&-15_001, &5_000, &RoundingMode::RoundHalfUp),
      -7_501
    );

    for bps in [1_u32, 5_000, 9_999, 10_000, 10_001] {
      let pos =
        client.compute_share(&i128::MAX, &bps, &RoundingMode::RoundHalfUp);
      let neg =
        client.compute_share(&i128::MIN, &bps, &RoundingMode::RoundHalfUp);
  ...
  pub fn replace_deferred(
    ...
    if env
      .storage()
      .persistent()
      .has(&DeferredDataKey::DeferredReports(period_id))
    {
      env
        .storage()
        .persistent()
  ...
  pub fn close_period(env: soroban_sdk::Env, period_id: u32) {
    ...
    if let Some(amount) =
      env.storage().persistent().get::<_, i128>(&deferred_key)
    {
      env.storage().persistent().remove(&deferred_key);
      env
        .events()
        .publish((soroban_sdk::symbol_short!("def_flush"), period_id), amount);
    }
  }
}
...
impl RevoraRevenueShare {
  ...
  pub fn enqueue_deferred(
    ...
    let mut entries: Vec<DeferredQueueEntry> = env
      .storage()
      .persistent()
      .get(&queue_key)
      .unwrap_or_else(|| Vec::new(&env));
    ...
    let new_entry = DeferredQueueEntry {
      release_ts,
      priority,
      queue_id,
      payload_id,
...
impl RevoraRevenueShare {
  ...
  pub fn create_gov_proposal(
    ...
    env.storage().persistent().set(
      &DataKey2::GovProposal(offering_id.clone(), proposal_id),
      &proposal,
    );
    env
      .storage()
      .persistent()
      .set(&count_key, &proposal_id.saturating_add(1));
  ...
  pub fn cast_vote(
    ...
    let mut proposal: GovProposalEntry = env
      .storage()
      .persistent()
      .get(&prop_key)
      .ok_or(RevoraError::LimitReached)?;
    ...
    let vote_key =
      DataKey2::VoteRecord(offering_id.clone(), proposal_id, voter.clone());
...
impl RevoraRevenueShare {
  ...
  pub fn clear_migration_hook(
    ...
    env
      .events()
      .publish((EVENT_MIG_HOOK_APPLIED, symbol_short!("clear")), legacy_key);
  ...
  pub fn get_registered_hooks(env: Env) -> Vec<MigrationHook> {
    ...
    for i in 0..count {
      if let Some(key) =
        env.storage().persistent().get::<MigrationDataKey, Symbol>(
          &MigrationDataKey::MigrationHookIndex(i),
        )
      {
        if let Some(transform) = env
          .storage()
          .persistent()
          .get::<MigrationDataKey, MigrationTransform>(
  ...
  pub fn verify_merkle_proof(
    ...
    use crate::merkle_helpers::{
      verify_merkle_proof as merkle_verify_proof, MAX_PROOF_DEPTH,
    };
  ...
  pub fn migrate_storage_walker(
    ...
    let mut cursor: MigrationCursor = env
      ...
      .persistent()
      .get(&cursor_key)
      .unwrap_or(MigrationCursor { last_key: 0 });

    if cursor.last_key > 0 && !dry_run {
      env.events().publish(
        (symbol_short!("mig_rsme"), from_version, to_version),
        cursor.last_key,
      );
    }

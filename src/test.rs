...
fn set_and_get_platform_fee() {
    ...
    client.initialize(&admin, &None::<Address>);
    client.set_platform_fee(&250).unwrap();
...
fn set_platform_fee_to_zero() {
    ...
    client.initialize(&admin, &None::<Address>);
    client.set_platform_fee(&500).unwrap();
    client.set_platform_fee(&0).unwrap();
    assert_eq!(client.get_platform_fee(), 0);
}
...
fn set_platform_fee_to_maximum() {
    ...
    client.initialize(&admin, &None::<Address>);
    client.set_platform_fee(&5_000).unwrap();
...
fn set_platform_fee_above_maximum_fails() {
    ...
    client.initialize(&admin, &None::<Address>);
    let result = client.try_set_platform_fee(&5_001);
    assert!(result.is_err());
}
...
fn update_platform_fee_multiple_times() {
    ...
    client.set_platform_fee(&100).unwrap();
    assert_eq!(client.get_platform_fee(), 100);

    client.set_platform_fee(&300).unwrap();
...
fn calculate_platform_fee_basic() {
    ...
    client.initialize(&admin, &None::<Address>);
    client.set_platform_fee(&250).unwrap(); // 2.5%
...
fn calculate_platform_fee_at_maximum_rate() {
    ...
    client.initialize(&admin, &None::<Address>);
    client.set_platform_fee(&5_000).unwrap(); // 50%
...
fn calculate_platform_fee_precision() {
    ...
    client.initialize(&admin, &None::<Address>);
    client.set_platform_fee(&1).unwrap(); // 0.01%
...
fn platform_fee_only_admin_can_set() {
    ...
    // Admin can set
    assert!(client.try_set_platform_fee(&100).is_ok());
    
    // After re-initializing with new admin, only new admin should be able to set
    // (but we can't re-initialize, so this test just verifies admin can set)
}
...
fn platform_fee_integration_with_revenue() {
    ...
    client.initialize(&admin, &None::<Address>);
    client.set_platform_fee(&500).unwrap(); // 5%
    client.register_offering(&issuer, &token, &5_000, &payout_asset).unwrap();

    // Calculate expected fee
    let revenue = 1_000_000_i128;
    let expected_fee = client.calculate_platform_fee(&revenue);
    assert_eq!(expected_fee, 50_000); // 5% of 1M
    
    // Net amount would be 950_000 for distribution
    let net = revenue - expected_fee;
